...
## ขั้นตอนบังคับ 7 ขั้น (Owner อนุมัติแล้ว — ห้ามข้าม)

ทุกงานที่พี่สั่ง ต้องวิ่งผ่าน 7 ขั้นนี้ตามลำดับเสมอ:

1. **วางแผน** — ฟังเป้าหมายจากพี่ให้ตรงกัน เขียนแผนลงการ์ดงานใน TASKS.md
2. **หา AI มาช่วย** — เมื่องานต้องการสมอง/โมเดลเพิ่ม (ต้องหาโมเดลใหม่ หรืองานเสี่ยง)
   ให้ส่ง model-recruiter ไปค้นหา/คัดเลือกโมเดลที่เหมาะสมกับงานตาม MODEL_POLICY
   ยังไม่เลือกใช้โมเดลเองโดยพลการ
3. **รายงานตัวเลือก** — model-recruiter รายงาน: โมเดลที่แนะนำ + เหตุผล + ราคา +
   backup ยังไม่เริ่มทำงาน
4. **รอOwner อนุมัติ** — นำตัวเลือกให้พี่ตัดสิน ยังไม่เรียกโมเดลนานั้นมาช่วยงานก่อนอนุมัติ
5. **เรียกมาช่วยตามระเบียบ** — เมื่อ approved ให้มอบหมายงานผ่าน START_PROMPT (INTAKE
   ก่อน / DELIVERY หลัง) ตาม AI_OPERATING_PROTOCOL เสมอ
6. **ตรวจและรวมผล** — project-lead ตรวจหลักฐานจากทุกตัวที่ทำงาน ไม่ทับซ้อน รวมผลเป็น
   ภาพเดียว ให้ reviewer ตรวจงานสำคัญก่อนสรุป
7. **ส่งมอบให้ Owner ตรวจ** — รายงานรวมภาษาไทยสั้น ๆ เสร็จอะไร + หลักฐาน + ความเสี่ยง +
   ขั้นต่อไป ให้พี่ตรวจ

ถ้าอยากใช้โมเดลใน roster ที่อนุมัติแล้วโดยไม่หาใหม่ ให้แจ้งพี่ชัดว่าใช้ตัวไหนจาก roster
(อ้าง MODEL_ROSTER.md) — ไม่ต้องเรียก model-recruiter ซ้ำ
