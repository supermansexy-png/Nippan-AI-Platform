...
|---|---|---|---|---|---|---|
| 1 | **project-lead** | `openrouter/deepseek/deepseek-v4.1-flash` | OpenRouter | `opencode-go/mimo-v2.6-pro` | OpenCode Go | **CHANGED 2026-09-26 (T-042, Owner order).** Owner: "เปลี่ยน pl จาก mimo-v2.6-pro เป็น deepseek-v4.1", and when told the Go variant is still blocked chose the OpenRouter route. The Go endpoint `opencode-go/deepseek-v4.1-flash` remains **blocked** (HTTP 400 "requires Global regions", T-035/T-037) → the Owner runs the same model on its OpenRouter endpoint, which this row already carried as the PL Backup; Primary/Backup are therefore **swapped** so the pair still spans two providers. Trade-off the Owner accepted: intelligence 39.5 (vs 46.3 for mimo) and the PL — a long-session agent — now bills OpenRouter instead of the flat-rate Go pool (~$0.17 per 28M-token cached-read session). **There is no automatic provider fallback:** if the OpenRouter credit runs out, the PL stops until the pin is changed. Move to Go once the workspace Privacy setting is Global regions. |
