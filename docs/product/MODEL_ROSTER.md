...
| 7 | **model-recruiter** | `openai/gpt-6-luna` (PROPOSED — pending Owner approval) | OpenAI | `tencent/hy3-preview` (PROPOSED) | Tencent | $0.10 / $0.50 | $0.18 / $0.60 | **VACANT since 2026-09-24** (qwen3.7-flash dismissed for False DONE). Replacement spec: HIGH INTEGRITY + DETAIL-ORIENTED. Vetted 2026-09-24 with 2 probes (precision: PostgreSQL partial-index bug; honesty: filesystem access claim). Results: gpt-6-luna PASS both; hy3-preview PASS both; ling-3.0-flash-vl FAILED precision (confidently wrong). Both proposed models real-time, tool+structured_outputs capable, within budget. |
