...
do $$ begin
  ...
  begin insert into public.project_rooms (tenant_id, application_id, project_key, title, mode, created_by_principal_id, automatic_round_limit, token_budget) values ((select value from wr_ids where key='tenant_a'),(select value from wr_ids where key='app_a'),'bad-round','Bad','FORMAL_MEETING','owner-a',3,1); raise exception 'round >2 accepted'; exception when check_violation then null; end;
